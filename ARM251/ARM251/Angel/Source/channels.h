...
/*
 * The following globals are only valid after angel_InitialiseChannels()
 * has been called.
 */
...
/* TRUE if heartbeats should be acked, FALSE otherwise. */
extern unsigned Angel_AckedHeartbeats;
...
/*
 * Function: Angel_InitChannels, Angel_ReinitChannels
 *  Purpose: initialise sequence numbers and free anyt leftover buffers. Init
 *           starts from the premise that nothing is kept; reinit just frees
 *           used things (i.e. it is not a total reset). Both reset the sequence
 *           numbers.make
...
AdpErrs Angel_InitChannels(void);
AdpErrs Angel_ReinitChannels(void);

/*
 * Function: adp_translate_dev_error
 *  Purpose: translate a device error code (e.g. DE_INVAL) into the
 *           equivalent adp error code.
 *
 *   Params:
 *              Input: - dev error
 *             Output: - adp error
...
AdpErrs adp_translate_dev_error(DevError dev_err);
...
typedef void (*ChanTx_CB_Fn)(unsigned taskid,           /* which task */
                             ChannelID  chanid,         /* which channel  */
...
typedef void (*ChanRx_CB_Fn)(unsigned taskid,    /* which task */
                             DeviceID   devID,   /* ID of receiving device  */
...
word angel_CanAckHeartbeat(void);
