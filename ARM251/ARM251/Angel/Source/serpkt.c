...
serpkt_rxflow(DeviceID devid, void *v_arg)
{
    ...
    LogInfo(LOG_SERPKT, ( "serpkt_rxflow: dev %d arg %x\n", devid, arg));
...
serpkt_int_rx_processing(void *args)
{
    ...
    while (ringBufNotEmpty(ring))
    {
        ...
        switch (rx_eng_status)
        {
            ...
            case RS_GOOD_PKT:
                INC_STAT_BY(spk_rx_numbytes, rxp->len);
                INC_STAT(spk_rx_numpackets);
