...
struct ChanState
{
    ...
    unsigned     read_task;            /* for blocking reads */
    ...
    unsigned     write_task;           /* task id of waiting task */
};
    ...
    switch (status)
    {
        ...
        case DS_DONE:
                ...
                /* packet accepted ; send it off ... */
            {
                ...
                {
                    chan_state[chanid].reader(chan_state[chanid].read_task,
                                              devid, chanid, data_pkt, data_len,
                                              chan_state[chanid].read_cb_data);
                }
                else if (chanid == read_all_channel && read_all_callback != NULL)
                {
                    read_all_callback(read_all_task, devid, chanid, data_pkt, data_len,
                                      read_all_cb_data);
                }
                else
                {
...
static void 
write_unblock_callback(unsigned taskid, ChannelID chanid, void *callback_data)
{
    ...
    *write_error = CE_OKAY;
    ...
    LogInfo(LOG_CHANNEL, ( "write_unblock_callback: task %d, channel %d\n", taskid, chanid));
    
    Angel_Signal(taskid, SIGBIT_WRITECB);
}
                  ...
                  const p_Buffer data_pkt, unsigned data_len)
{
     ...
     *}
     */
    sigs = Angel_Wait(SIGBIT_WRITECB);
               ...
               void *v_status, void *cb_data)
{
    ...
    if (status == DS_DONE)
    {
        ...
        {
            ChanTx_CB_Fn writer = chan_state[chanid].writer;
            unsigned task = chan_state[chanid].write_task;
            void *cb = chan_state[chanid].write_cb_data;
            
            LogInfo(LOG_CHANNEL, ("write_callback: chan %d callback\n", chanid));
            
            writer(task, chanid, cb);
            
            LogInfo(LOG_CHANNEL, ("write_callback: chan %d cleanup\n", chanid));
            
            chan_state[chanid].writer = NULL;
            chan_state[chanid].write_task = 0;
                       ...
                       void *callback_data)
{
    ...
    chan_state[chanid].write_task = Angel_TaskID();
...
resend_packet_from(DeviceID devid, SequenceNR startpacket)
{
           ...
           adp_between(startpacket, thispacket, nextSEQ(dev->currentFrame)))
    {
        ...
        if ((sigs & SIGBIT_WRITECB) == 0)
            LogError(LOG_CHANNEL, ( "resend msg: wait returned %x\n", sigs));
...
static void 
read_unblock_callback(unsigned taskid, DeviceID devid, ChannelID chanid,
                      ...
                      void *callback_data)
{
    ...
    *read_error = CE_OKAY;
    ...
    Angel_Signal(taskid, SIGBIT_READCB);
}
                  ...
                  unsigned *len)
{
    ...
    chan_state[chanid].read_task = Angel_TaskID();
                          ...
                          unsigned *len)
{
    ...
    do
    {
        sigs |= Angel_Wait(SIGBIT_READCB | SIGBIT_WRITECB);
        
    } while (sigs != (SIGBIT_READCB | SIGBIT_WRITECB));
