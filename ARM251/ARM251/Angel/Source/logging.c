...
#ifdef __unix
#include <sys/time.h>
#if defined(sun) && !defined(COMPILING_ON_SOLARIS)
                ...
                unsigned int length)
{
    ...
    if (length > 256)
    {
        log_log(lvl, "Buffer 0x%x, length %d (truncated to 256)\n",
                buffer, length);
        length = 256;
    }
    else
    {
        log_log(lvl, "Buffer 0x%x, length %d\n", buffer, length);
    }
    ...
    while ((format != NULL) && (*format != '\0'))
    {
        if (*format == '%')
        {
            ...
            switch (fch)
            {
                ...
                case '\0':
                    /*
                     * string terminated by '%' character, 
                     * prepare for default "format++" below
                     */
                    format--;
