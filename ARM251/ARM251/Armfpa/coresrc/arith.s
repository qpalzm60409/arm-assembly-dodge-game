...
;
; RCS $Revision: 1.4.32.1 $
; Checkin $Date: 1998/08/14 13:26:07 $
...
; entry point is given two labels, namely "FltFPE" and "FltSC".
...
FltSC
        ...
        CDebug1 3,"FltFPE/SC: operand =",Rarith
...
; optimised fast track for both operands being common, and "AddSubSC",
...
; IEEE sign rule (i.e. -0 if rounding to -infinity, otherwise +0).
        ...
        BLNE    NormaliseOp1            ;If not, complete normalisation
        LDMNEFD Rsp!,{PC}               ; and return (note NormaliseOp1
                                        ; returns NE if called)
...
; track for both operands being common, and "MultSC", which avoids the
...
; "DivSC", which avoids the test for this optimised fast track - since it
...
RemSC
        ...
        CDebug3 3,"RemSC/FPE: op1 =",OP1sue,OP1mhi,OP1mlo
        CDebug3 3,"           op2 =",OP2sue,OP2mhi,OP2mlo
...
; first operand.
        ...
        ; We will use the current 0 in the top of Rarith as the sign bit
...
; the least significant bit of the unbiased exponent into C.
        ...
        MOVNE   Rarith,OP1sue           ;Probable sign bit generation
                                        ; method for any future hardware
...
RndSC
        ...
        CDebug3 3,"RndSC/FPE: operand =",OP1sue,OP1mhi,OP1mlo
...
; vs. common comparisons, and "CompareSC", which avoids the test for this
...
AddSubSC
        ...
        CDebug3 3,"AddSubSC: op1 =",OP1sue,OP1mhi,OP1mlo
        CDebug3 3,"          op2 =",OP2sue,OP2mhi,OP2mlo
...
; apply.
        ...
        BIC     Rarith,Rins,Rins,LSR #1         ; operation; normal sign bit
        MOV     Rarith,Rarith,LSL #(30-RM_pos)  ; is set for "to -infinity"
        ASSERT  RM_MinusInf = 2:SHL:RM_pos      ; rounding, clear otherwise
        B       InvalidOp2ForSDE
...
MultSC
        ...
        CDebug3 3,"MultSC: op1 =",OP1sue,OP1mhi,OP1mlo
        CDebug3 3,"        op2 =",OP2sue,OP2mhi,OP2mlo
...
DivSC
        ...
        CDebug3 3,"DivSC: op1 =",OP1sue,OP1mhi,OP1mlo
        CDebug3 3,"       op2 =",OP2sue,OP2mhi,OP2mlo
...
; not URD results and we've already dealt with extended infinities.
        ...
        ORRS    Rarith,OP2mhi,OP2mlo
        MOVEQ   Rtmp,#InvReas_XRem0
        ; We will simply use the current 0 in the top of Rarith as the sign
        ; bit - we don't really care what it is.
        BEQ     InvalidOp2ForSDE
...
; been dealt with already.
        ...
        BLEQ    NormaliseOp2                    ; then adjust to get
        ...
        BLEQ    NormaliseOp1                    ; then determine the number
...
; apply.
        ...
        MOV     Rarith,#0                       ;Just to make the sign predictable
        B       InvalidOp2ForSDE
...
; have been dealt with already.
        ...
        B       NormaliseOp1            ;NB must be necessary, so no
                                        ; point in checking whether
                                        ; normalised
...
MoveSC
        ...
        CDebug3 3,"MoveSC: operand =",OP1sue,OP1mhi,OP1mlo
...
Move_NaN
        ...
        STMFD   Rsp!,{LR}
        [ SigNaNCopy_OK                 ;Otherwise, conversion always wanted
...
NormSC
        ...
        CDebug3 3,"NormSC: operand =",OP1sue,OP1mhi,OP1mlo
...
UrdSC
        ...
        CDebug3 3,"UrdSC: operand =",OP1sue,OP1mhi,OP1mlo
...
CompareSC
        ...
        CDebug3 3,"CompareSC: op1 =",OP1sue,OP1mhi,OP1mlo
        CDebug3 3,"           op2 =",OP2sue,OP2mhi,OP2mlo
...
; the two operands.
        ...
        BEQ     InvalidOp2ForI                  ; (invalid operation if so)
...
FixSC
        ...
        CDebug3 3,"FixSC: operand =",OP1sue,OP1mhi,OP1mlo
...
;   The potential integer result is made 0x7FFFFFFF for +infinity,
; 0x80000000 for -infinity, and 0x00000000 for NaNs - this is for
; compatibility with the results Java expects.

        TST     OP1mhi,#EIFracTop_bit
        MOVEQ   Rtmp,#InvReas_SigNaN
        MOVNE   Rtmp,#InvReas_FixQNaN
        ORRS    Rarith,OP1mlo,OP1mhi,LSL #1
        MOVNE   Rarith,#0                       ; Result for a NaN
        MOVEQ   Rtmp,#InvReas_FixInf
        MOVEQ   Rarith,#0x80000000              ; Result for -infinity
        TSTEQ   OP1sue,#Sign_bit                ; But if +infinity, change
        MVNEQ   Rarith,Rarith                   ;  result to be correct
        B       InvalidOp1ForI
