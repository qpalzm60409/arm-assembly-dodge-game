...
/*
 * RCS $Revision: 1.19 $
 * Checkin $Date: 1998/05/19 14:44:35 $
...
/* calls to getc/putc must be fflush/fseek separated.              */
    ...
    if ((fh = _sys_open(name, openmode)) == _sys_nonhandle()) return NULL;
...
void _initio(char *f1,char *f2,char *f3)
{
    ...
    if (setvbuf(stdin, NULL, _IOLBF, STDIN_BUFSIZ) != 0) __open_fail(f1);
    if (setvbuf(stdout, NULL, _IOLBF, STDOUT_BUFSIZ) != 0) __open_fail(f2);
    if (setvbuf(stderr, NULL, _IOLBF, STDERR_BUFSIZ) != 0) __open_fail(f3);
}
...
dbmsg("flushbuf negative ocnt = %d\n", stream->ocnt);
    ...
    {   _seterr(stream);
        ReferenceSymbol(_main_redirection);
dbmsg("%s\n", "!= _IOWRITE");
        return EOF;
    }
