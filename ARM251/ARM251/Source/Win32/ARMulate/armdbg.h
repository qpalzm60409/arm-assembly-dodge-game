 ...
 * Copyright (C) 1992-1998 Advanced Risc Machines Ltd. All rights reserved.
 * Copyright (C) 1998 ARM Limited. All rights reserved.
 */

/*
 * RCS $Revision: 1.132.2.6 $
 * Checkin $Date: 1998/11/11 16:26:34 $
...
Dbg_SymInfo *Dbg_GetSensibleName(Dbg_SymTable *st, ARMaddress addr);  /* BGC - New symbol search (this time for a sensible name!) */
...
typedef struct {
  ...
  unsigned int size;            /* size in bytes-all values are word-aligned */
...
Dbg_Error Dbg_DeclSpecToEnvironment(Dbg_MCState *state, Dbg_DeclSpec const *decl, Dbg_Environment *env);
...
#if RDI_VERSION!=150
/*
 * Functions to convert to/from a toolconf
 */
extern Dbg_Error Dbg_ToolconfToConfigBlock(
    Dbg_MCState *state, toolconf config, RDI_ConfigBlock *block);
extern Dbg_Error Dbg_ConfigBlockToToolconf(
    Dbg_MCState *state, toolconf tconf, const RDI_ConfigBlock *config);
#endif
